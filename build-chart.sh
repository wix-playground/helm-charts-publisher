...
then
    ...
    /tmp/helm/helm repo index build/ --merge prevdata/index.yaml --url http://helm-repo.sphera.tools.s3-website-us-east-1.amazonaws.com/cluster-autoscaler
else
    echo 'No previous charts to merge'
    /tmp/helm/helm repo index build/ --url http://helm-repo.sphera.tools.s3-website-us-east-1.amazonaws.com/cluster-autoscaler
fi